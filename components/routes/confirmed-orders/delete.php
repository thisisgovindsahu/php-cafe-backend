...
if ($_SERVER['REQUEST_METHOD'] === 'DELETE') {
    ...
    if (isset($_GET['confirmedOrderid'])) {
        $id = $_GET['confirmedOrderid'];

        try {
            // Prepare the DELETE query
            $deleteQuery = $conn->prepare("DELETE FROM confirms WHERE id = ?");
            ...
            if ($deleteQuery->rowCount() > 0) {
                ...
                echo json_encode([
                    "success" => false,
                    "message" => "Confirmed order not found or already deleted."
                ]);
            }
            ...
            echo json_encode([
                "success" => false,
                "message" => "Failed to delete confiremd order: " . $e->getMessage()
            ]);
        }
    } else {
        ...
        echo json_encode([
            "success" => false,
            "message" => "Confirmed order ID is required."
        ]);
    }
} else {
